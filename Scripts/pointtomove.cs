    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && GUIUtility.hotControl == 0)
        {
            ...
            if (playerPlane.Raycast(ray, out hitdist))
            {
                ...
                //playermodel.transform.rotation = Quaternion.Lerp(playermodel.transform.rotation, targetmodelrotation, Time.deltaTime * 500);
        ...
        else if (Input.GetMouseButton(0) && GUIUtility.hotControl == 0)
        {
            ...
            if (playerPlane.Raycast(ray, out hitdist))
            {
                ...
                destinationPosition = ray.GetPoint(hitdist);
                Quaternion targetRotation = Quaternion.LookRotation(targetPoint - transform.position);
                Quaternion targetmodelrotation = Quaternion.LookRotation(targetPoint - transform.position );
